export class CentralMedidas {
    ...
    mediaMedidasTotal(){
        if (this.#medidas.length === 0) return null;
    ...
    toHTML(){
        ...
        for (let i = 1; i <= 30; i++) html += `<th>Día ${i}</th>`;
        html += "<th>Media</th></tr>";

        for (const registro of this.#medidas) {
            const media = this.mediaMedidas(registro.ciudad);
            html += `<tr><td>${registro.ciudad}</td>`;
            for (const v of registro.valores) html += `<td>${v}</td>`;
            html += `<td><b>${media}</b></td></tr>`;
        }
